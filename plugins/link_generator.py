import logging
from pyrogram import Client, filters
from pyrogram.types import Message
from bot import Bot
from helper_func import encode, get_message_id

# Set up logging for debugging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

@Bot.on_message(filters.private & filters.user(ADMINS) & filters.command('batch'))
async def batch(client: Client, message: Message):
    try:
        while True:
            first_message = await client.ask(
                text="Forward the First Message from DB Channel (with Quotes)..\n\nor Send the DB Channel Post Link",
                chat_id=message.from_user.id,
                filters=(filters.forwarded | filters.regex(r"https://t.me/.+"))
            )

            f_msg_id = await get_message_id(client, first_message)
            if f_msg_id:
                break
            else:
                await first_message.reply("❌ Error\n\nThis Forwarded Post is not from my DB Channel or this Link is not from DB Channel", quote=True)

        while True:
            second_message = await client.ask(
                text="Forward the Last Message from DB Channel (with Quotes)..\nor Send the DB Channel Post link",
                chat_id=message.from_user.id,
                filters=(filters.forwarded | filters.regex(r"https://t.me/.+"))
            )

            s_msg_id = await get_message_id(client, second_message)
            if s_msg_id:
                break
            else:
                await second_message.reply("❌ Error\n\nThis Forwarded Post is not from my DB Channel or this Link is not from DB Channel", quote=True)

        string = f"get-{f_msg_id * abs(client.db_channel.id)}-{s_msg_id * abs(client.db_channel.id)}"
        base64_string = await encode(string)

        # Send debug message to confirm generation
        logger.info(f"Base64 String Generated: {base64_string}")

        # Generate the initial Telegram link
        initial_telegram_link = f"https://t.me/Adult_Video_Storej2_Bot?start={base64_string}&type=file"
        await second_message.reply_text(f"<strong>{initial_telegram_link}</strong>", quote=True)

        # Testing only the generated link
        @Bot.on_message(filters.regex(rf"https://t.me/Adult_Video_Storej2_Bot\?start={base64_string}&type=file$"))
        async def handle_blogspot(client: Client, message: Message):
            try:
                # Log the clicked URL
                logger.info(f"Received link: {message.text}")

                # Send the Blogspot link
                blogspot_link = f"https://jn2flix.blogspot.com/2025/01/adultx.html?JN2FLIX={base64_string}"
                await message.reply_text(f"<strong>\n\n{blogspot_link}\n\n</strong>")

            except Exception as e:
                logger.error(f"Error handling blogspot link: {e}")

    except Exception as e:
        logger.error(f"Error in batch process: {e}")


@Bot.on_message(filters.private & filters.user(ADMINS) & filters.command('genlink'))
async def link_generator(client: Client, message: Message):
    try:
        while True:
            channel_message = await client.ask(
                text="Forward Message from the DB Channel (with Quotes)..\nor Send the DB Channel Post link",
                chat_id=message.from_user.id,
                filters=(filters.forwarded | filters.regex(r"https://t.me/.+"))
            )

            msg_id = await get_message_id(client, channel_message)
            if msg_id:
                break
            else:
                await channel_message.reply("❌ Error\n\nThis Forwarded Post is not from my DB Channel or this Link is not from DB Channel", quote=True)

        base64_string = await encode(f"get-{msg_id}")
        generated_link = f"https://t.me/Adult_Video_Storej2_Bot?start={base64_string}&type=file"

        # Log generated link
        logger.info(f"Generated Link: {generated_link}")

        await channel_message.reply_text(f"<strong>Generated Link:</strong>\n{generated_link}", quote=True)

        # Handle click event and respond with blogspot link
        @Bot.on_message(filters.regex(rf"https://t.me/Adult_Video_Storej2_Bot\?start={base64_string}&type=file$"))
        async def handle_blogspot(client: Client, message: Message):
            try:
                blogspot_link = f"https://jn2flix.blogspot.com/2025/01/adultx.html?JN2FLIX={base64_string}"
                await message.reply_text(f"<strong>\n\n{blogspot_link}\n\n</strong>")
            except Exception as e:
                logger.error(f"Error handling blogspot link in genlink: {e}")

    except Exception as e:
        logger.error(f"Error in genlink process: {e}")
